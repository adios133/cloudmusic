...
<template>
  <div class="user-cart">
    ...
    <h4 class="username" @click="logIn">
      {{ userInfo.nickname ? userInfo.nickname : "游客" }}
    </h4>
    <div class="subscriber">
      <span class="followers"
        ><i>{{ userInfo.nickname ? userInfo.follows : "0" }}</i> 关注</span
      >
      <span class="fans"
        ><i>{{ userInfo.nickname ? userInfo.followeds : "0" }}</i> 粉丝</span
      >
      <i class="level">Lv.{{ userInfo.nickname ? userInfo.level : "0" }}</i>
    </div>
    <div class="signature">
      {{ userInfo.nickname ? userInfo.signature : "" }}
    </div>
  </div>
</template>
